  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        ...
        <a class="btn btn-primary" style="float: right" href="{{route('companies.create')}}">Add company</a>
      </div>
      <div class="card-body">
        <table id="tableContainer"  class="table table-bordered table-striped text-center">
          ...
          @foreach ($companies as $company)   
              <tr class="odd gradeX"  id="{{$company->id}}" >
                <td>{{$company->name_en}} </td>
                <td>{{$company->name_ar}} </td>
                <td><img src="{{$company->images->first()?$company->images->first()->full_url:asset("assets/admin.png")}}" class="img elevation-2" height="50" alt="User Image"></td>
  ...
  $(".remove").click(function(){
      ...
      swal.fire({
              ...
              if (result.isConfirmed) {
                $.ajax({
                      url: '{{url("/dashboard/companies")}}'+'/'+id,
                      type: 'Delete',
                      error: function() {
                        swal.fire({
